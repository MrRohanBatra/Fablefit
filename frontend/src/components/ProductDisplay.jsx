...
function ProductDisplay() {
  ...
  const handleAddToCart = async () => {
    if (!selectedSize) {
      showToast("Please select a size first.","danger",1000);
      return;
    }
    ...
    try {
      ...
      setCart(updatedCart);
      showToast(`${quantityToAdd} item(s) added to cart!`,"success",1000);
    } catch (err) {
      console.error("❌ Failed to add to cart:", err);
      showToast("Failed to add item to cart. Please try again.","danger",1000);
    }
  };
